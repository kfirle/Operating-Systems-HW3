...
static inline game_params parse_input_args(int argc, char **argv) {
    ...
    game_params g;
    g.filename = argv[1]; //"small.txt";
    g.n_gen = strtoul(argv[2], NULL, 10); //10;
    g.n_thread = strtoul(argv[3], NULL, 10); //4;

    string inter = string(argv[4]);
    string print = string(argv[5]);
    g.interactive_on = (inter == "y" || inter == "Y") ? true : false; //אtrue
    g.print_on = (print == "y" || print == "Y") ? true : false; //true;
